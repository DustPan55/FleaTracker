...
(() => {
  ...
  async function addTreatment(e) {
    ...
    const { data, error } = await sb.from("flea_treatments").insert(record).select().single();
    ...
    formMsg.textContent = `✓ Saved for ${(dogs.find((d) => d.id === data.dog_id) || {}).name || "dog"}`;
    fDose.value = "";
    fNotes.value = "";
  ...
  form.addEventListener("submit", addTreatment);
